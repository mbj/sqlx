...
pub struct MaybeUpgradeTls<'a>(pub &'a ConnectOptions);
...
async fn maybe_upgrade<S: Socket>(
    ...
    match options.ssl_mode {
        ...
        SslMode::Allow | SslMode::Disable => return Ok(Box::new(socket)),
    ...
    let accept_invalid_hostnames = !matches!(options.ssl_mode, SslMode::VerifyFull);
