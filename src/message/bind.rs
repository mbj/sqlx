use crate::io::{BufMutExt, PortalId, StatementId};
...
pub struct Bind<'a> {
    ...
    pub result_formats: &'a [ValueFormat],
}
...
impl FrontendMessage for Bind<'_> {
    ...
    fn encode_body(&self, buf: &mut Vec<u8>) -> Result<(), crate::Error> {
        ...
        let formats_len = u16::try_from(self.formats.len()).map_err(|_| {
            crate::err_protocol!("too many parameter format codes ({})", self.formats.len())
        })?;
        ...
        let result_formats_len = u16::try_from(self.formats.len())
            .map_err(|_| crate::err_protocol!("too many result format codes ({})", self.formats.len()))?;
