...
impl Money {
    ...
    /// See the type-level docs for an explanation of `locale_frac_digits`.
    ///
    /// Note that `Decimal` has 96 bits of precision, but `Money` only has 63 plus the sign bit.
...
impl HasArrayType for Money {
    fn array_type_info() -> TypeInfo {
        TypeInfo::MONEY_ARRAY
    }
}
...
mod tests {
    ...
    #[test]
    fn adding_works() {
        assert_eq!(Money(3), Money(1) + Money(2))
    }

    #[test]
    fn add_assign_works() {
        let mut money = Money(1);
        money += Money(2);

        assert_eq!(Money(3), money);
    }

    #[test]
    fn subtracting_works() {
        assert_eq!(Money(4), Money(5) - Money(1))
    }
    ...
    fn sub_overflow_panics() {
        let _ = Money(i64::MIN) - Money(1);
    }

    #[test]
    #[should_panic]
    fn sub_assign_overflow_panics() {
        let mut money = Money(i64::MIN);
        money -= Money(1);
    }
    ...
    fn conversion_from_decimal_works() {
        assert_eq!(
            Money(12345),
            Money::from_decimal(rust_decimal::Decimal::new(12345, 2), 2)
        );

        assert_eq!(
            Money(12345),
            Money::from_decimal(rust_decimal::Decimal::new(123450, 3), 2)
        );
        ...
        assert_eq!(
            Money(-12300),
            Money::from_decimal(rust_decimal::Decimal::new(-123, 0), 2)
        );
    }
    ...
    fn conversion_from_bigdecimal_works() {
        ...
        assert_eq!(Money(12345), Money::from_bigdecimal(dec, 2).unwrap());
    }
}
