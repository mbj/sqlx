...
> released, as well as PRs we depend on that may not yet have been reviewed or merged upstream.
> We maintain this fork for our own projects and share it for others who need these fixes.
>
> ### Versioning and Branch Strategy
>
> - **Versions** follow the scheme `0.9.0-msqlx.N`, where `0.9` tracks the upstream branch we're
>   based on, and `N` is our incremental release number.
> - **Tags are immutable**: once a version like `v0.9.0-msqlx.1` is tagged and published, it is
>   never moved or deleted.
> - **The `msqlx-main` branch is regularly rewritten**: we rebase on upstream and force-push to
>   keep the branch clean. Do not depend on its commit history — use tagged releases instead.
>
> ### Key Points
>
...
> ```toml
> [dependencies]
> sqlx = { package = "msqlx", version = "0.9.0-msqlx.2", features = ["runtime-tokio", "postgres"] }
> ```
>
