...
/// ```
///
/// However, this may cause an error if the type being wrapped does not implement `HasArrayType`,
/// e.g. `Vec` itself, because we don't currently support multidimensional arrays:
///
/// ```rust,ignore
/// #[derive(sqlx::Type)] // ERROR: `Vec<i64>` does not implement `HasArrayType`
/// #[sqlx(transparent)]
/// struct UserIds(Vec<i64>);
/// ```
///
/// To remedy this, add `#[sqlx(no_pg_array)]`, which disables the generation
/// of the `HasArrayType` impl:
///
/// ```rust,ignore
/// #[derive(sqlx::Type)]
...
impl<T> HasArrayType for Option<T>
where
    T: HasArrayType,
{
    fn array_type_info() -> TypeInfo {
...
impl<T> HasArrayType for Text<T> {
    fn array_type_info() -> TypeInfo {
...
impl<'q, T, const N: usize> Encode<'q> for [T; N]
where
    for<'a> &'a [T]: Encode<'q>,
    T: Encode<'q>,
{
    ...
    T: Encode<'q> + Type,
{
    fn encode_by_ref(&self, buf: &mut ArgumentBuffer) -> Result<IsNull, BoxDynError> {
        ...
        crate::BindIterExt::bind_iter(self.iter()).encode(buf)
    }
}

impl<'r, T, const N: usize> Decode<'r> for [T; N]
where
    ...
    fn decode(value: ValueRef<'r>) -> Result<Self, BoxDynError> {
        ...
        match format {
            ValueFormat::Binary => {
                ...
                let element_type_info: TypeInfo = TypeInfo::try_from_oid(element_type_oid)
                ...
                for _ in 0..len {
                    let value_ref = ValueRef::get(&mut buf, format, element_type_info.clone())?;
            ...
            ValueFormat::Text => {
                ...
                while !done {
                    ...
                    elements.push(T::decode(ValueRef {
                        value: value_opt,
                        row: None,
