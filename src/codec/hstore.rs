...
use crate::{
    ...
    error::BoxDynError,
    codec::Type,
    ArgumentBuffer, HasArrayType, TypeInfo, ValueRef,
};
use crate::bytes::Buf;
...
///
/// ```
/// # use sqlx::codec::Hstore;
/// // Shows basic usage of the Hstore type.
/// //
...
/// ```
/// # use sqlx::codec::Hstore;
/// // Hstore implements FromIterator to simplify construction.
/// //
/// let additional_data = Hstore::from_iter([
...
impl Type for Hstore {
    fn type_info() -> TypeInfo {
        TypeInfo::with_name("hstore")
    }
}

impl HasArrayType for Hstore {
    fn array_type_info() -> TypeInfo {
        TypeInfo::array_of("hstore")
    }
}

impl<'r> Decode<'r> for Hstore {
    fn decode(value: ValueRef<'r>) -> Result<Self, BoxDynError> {
        let mut buf = <&[u8] as Decode>::decode(value)?;
        let len = read_length(&mut buf)?;

        let len =
            usize::try_from(len).map_err(|_| format!("Hstore: length out of range: {len}"))?;

        let mut result = Self::default();

        for i in 0..len {
            let key = read_string(&mut buf)
                .map_err(|e| format!("Hstore: error reading {i}th key: {e}"))?
                .ok_or_else(|| format!("Hstore: expected {i}th key, got nothing"))?;

            let value = read_string(&mut buf)
                .map_err(|e| format!("Hstore: error reading value for key {key:?}: {e}"))?;
    ...
    fn encode_by_ref(&self, buf: &mut ArgumentBuffer) -> Result<IsNull, BoxDynError> {
        ...
        for (i, (key, val)) in self.0.iter().enumerate() {
            ...
            let key_len = i32::try_from(key_bytes.len()).map_err(|_| {
                ...
                format!(
                    "Hstore: length of {i}th key out of range: {} bytes",
                    key_bytes.len()
                )
            })?;
            ...
            match val {
                Some(val) => {
                    ...
                    let val_len = i32::try_from(val_bytes.len()).map_err(|_| {
                        format!(
                            "Hstore: value length for key {key:?} out of range: {} bytes",
                            val_bytes.len()
                        )
                    })?;
...
mod test {
    ...
    fn hstore_deserialize_ok() {
        ...
        let empty = ValueRef {
            ...
            row: None,
            type_info: TypeInfo::with_name("hstore"),
            format: ValueFormat::Binary,
        };

        let name_surname = ValueRef {
        ...
        let res_empty = Hstore::decode(empty).unwrap();
        let res_name_surname = Hstore::decode(name_surname).unwrap();
    ...
    #[test]
    #[should_panic(expected = "Hstore: length out of range: -5")]
    fn hstore_deserialize_buffer_length_error() {
        ...
        Hstore::decode(buf).unwrap();
    }

    #[test]
    fn hstore_serialize_ok() {
        let mut buff = ArgumentBuffer::default();
        let _ = Hstore::from_iter::<[(String, String); 0]>([])
