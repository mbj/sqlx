...
/// ```
///
/// Given a query that returns the column as a signed 8-byte integer, `try_from = "i64"`
/// will convert it into the field's `u64` type.
///
/// #### `json`
///
...
/// represented as the _not_ NULL (in terms of crate::Postgres) JSON value of `null`.
///
...
///
/// ```rust,ignore
/// use sqlx::{postgres::Row, FromRow, Row};
/// struct MyCustomType {
///     custom: String,
/// }
///
...
impl<'r> FromRow<'r> for () {
    #[inline]
    fn from_row(_: &'r Row) -> Result<Self, Error> {
        Ok(())
    }
}
...
macro_rules! impl_from_row_for_tuple {
    ($( ($idx:tt) -> $T:ident );+;) => {
        ...
        where
            $($T: crate::decode::Decode<'r> + crate::codec::Type,)+
        {
            #[inline]
            fn from_row(row: &'r Row) -> Result<Self, Error> {
