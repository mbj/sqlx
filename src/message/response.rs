...
impl TryFrom<&str> for Severity {
    ...
    fn try_from(s: &str) -> Result<Severity, Error> {
        let result = match s {
            ...
            severity => {
                return Err(crate::err_protocol!("unknown severity: {:?}", severity));
            }
        };
...
impl ProtocolDecode<'_> for Notice {
    fn decode_with(buf: Bytes, _: ()) -> Result<Self, Error> {
        ...
        const DEFAULT_SEVERITY: Severity = Severity::Log;
...
fn test_decode_error_response() {
    ...
    assert!(matches!(m.severity(), Severity::Notice));
