...
pub struct Query<'q, A> {
    pub(crate) statement: Either<SqlStr, &'q crate::Statement>,
...
impl Query<'_, crate::Arguments> {
    ...
    pub fn bind<'t, T: Encode<'t> + Type>(mut self, value: T) -> Self {
    ...
    pub fn try_bind<'t, T: Encode<'t> + Type>(
        &mut self,
        value: T,
    ) -> Result<(), BoxDynError> {
    ...
    fn get_arguments(&mut self) -> Result<&mut crate::Arguments, BoxDynError> {
    ...
    A: 'q + IntoArguments,
{
    ...
    pub fn map<F, O>(
        self,
        mut f: F,
    ) -> Map<'q, impl FnMut(crate::Row) -> Result<O, Error> + Send, A>
    where
        F: FnMut(crate::Row) -> O + Send,
        O: Unpin,
    {
    ...
    where
        F: FnMut(crate::Row) -> Result<O, Error> + Send,
        O: Unpin,
    {
        Map {
            inner: self,
    ...
    #[inline]
    pub async fn execute<'c>(self, executor: &'c mut Connection) -> Result<crate::QueryResult, Error>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    ) -> BoxStream<'c, Result<crate::QueryResult, Error>>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    #[inline]
    pub fn fetch<'c>(self, executor: &'c mut Connection) -> BoxStream<'c, Result<crate::Row, Error>>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    pub fn fetch_many<'c>(
        self,
        executor: &'c mut Connection,
    ) -> BoxStream<'c, Result<Either<crate::QueryResult, crate::Row>, Error>>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    /// e.g. using `LIMIT`.
    #[inline]
    pub async fn fetch_all<'c>(self, executor: &'c mut Connection) -> Result<Vec<crate::Row>, Error>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    #[inline]
    pub async fn fetch_one<'c>(self, executor: &'c mut Connection) -> Result<crate::Row, Error>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    #[inline]
    pub async fn fetch_optional<'c>(self, executor: &'c mut Connection) -> Result<Option<crate::Row>, Error>
    where
        'q: 'c,
        A: 'c,
    {
    ...
    A: 'q + Send + IntoArguments,
{
    ...
    pub fn map<G, P>(
        self,
        mut g: G,
    ) -> Map<'q, impl FnMut(crate::Row) -> Result<P, Error> + Send, A>
    where
        G: FnMut(O) -> P + Send,
        P: Unpin,
    {
    ...
    ///
    /// Otherwise, you might want to add `LIMIT 1` to your query.
    pub async fn fetch_optional<'c>(mut self, executor: &'c mut Connection) -> Result<Option<O>, Error>
    where
        'q: 'c,
        F: 'c,
        O: 'c,
...
pub fn query_statement(statement: &crate::Statement) -> Query<'_, crate::Arguments>
where
{
    Query {
...
pub fn query_statement_with<A>(statement: &crate::Statement, arguments: A) -> Query<'_, A>
where
    A: IntoArguments,
{
    Query {
...
/// // where `conn` is `Connection` or some other type that implements `Executor`.
...
///
/// Postgres uses `$1`, `$2`, `$3`, etc. as placeholders:
///
...
///
/// The placeholder syntax acts as a variable expression representing the bound value:
///
/// ```rust,no_run
...
/// Any type that implements [`Encode`] and [`Type`] can be bound as a parameter.
///
/// See [the `types` module][crate::types] (links to `$crate::codec` but you should use `sqlx::types`) for details.
///
...
pub fn query<'a>(sql: impl SqlSafeStr) -> Query<'a, crate::Arguments>
where
{
    Query {
        arguments: Some(Ok(Default::default())),
...
pub fn query_with<'q, A>(sql: impl SqlSafeStr, arguments: A) -> Query<'q, A>
where
    A: IntoArguments,
{
