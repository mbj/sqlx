...
pub use response::{Notice, Severity};
...
impl ReceivedMessage {
        ...
        T: BackendMessage,
    {
        ...
        T::decode_body(self.contents).map_err(|e| match e {
            Error::Protocol(s) => {
                crate::err_protocol!("Postgres protocol error (reading {:?}): {s}", self.format)
            }
            other => other,
        })
    }
...
impl BackendMessageFormat {
    pub fn try_from_u8(v: u8) -> Result<Self, Error> {
        ...
        Ok(match v {
            ...
            _ => return Err(crate::err_protocol!("unknown message type: {:?}", v as char)),
        })
    }
}
