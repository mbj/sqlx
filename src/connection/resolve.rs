...
use crate::codec::{meta, CustomType, TypeKind};
use crate::codec::Oid;
use crate::{HashMap, Row, ValueRef};
use crate::{Column, Connection, TypeInfo};
use crate::column::{ColumnOrigin, TableColumn};
use crate::decode::Decode;
use crate::error::BoxDynError;
use crate::from_row::FromRow;
use crate::raw_sql::raw_sql;
...
impl Connection {
    pub(super) async fn resolve_statement_metadata<const QUERIES_ALLOWED: bool>(
    ...
    ) -> Result<Arc<StatementMetadata>, Error> {
        ...
        for ty in param_types {
            if let Some(type_info) = self.try_oid_to_type(ty) {
                ...
                parameters.push(TypeInfo(meta::Type::DeclareWithOid(ty)));
            }
        }
        ...
        for field in fields {
            ...
            let type_info = self
                ...
                .unwrap_or(TypeInfo(meta::Type::DeclareWithOid(field.data_type_id)));
    ...
    pub(crate) async fn resolve_types(&mut self, types: &[TypeInfo]) -> Result<Vec<Oid>, Error> {
        ...
        for ty in unresolved_types.clone() {
            ...
            if let meta::Type::DeclareArrayOf(array_of) = &ty.0 {
    ...
    pub(crate) fn try_type_to_oid(&self, ty: &TypeInfo) -> Option<Oid> {
        if let Some(oid) = ty.try_oid() {
            return Some(oid);
        }

        match &ty.0 {
            meta::Type::DeclareWithName(name) => self.inner.cache_type_oid.get(name).copied(),
            meta::Type::DeclareArrayOf(array) => {
    ...
    fn try_oid_to_type(&self, oid: Oid) -> Option<TypeInfo> {
        TypeInfo::try_from_oid(oid).or_else(|| self.inner.cache_type_info.get(&oid).cloned())
    }
    ...
    fn try_cache_type(&mut self, ty: &TypeResolverRow) -> Result<ControlFlow<Oid>, Error> {
        ...
        let custom_type_kind = match (ty.typtype, ty.typcategory) {
            ...
            (TypType::Pseudo, _) => TypeKind::Pseudo,
            ...
            (TypType::Range, _) => {
                ...
                TypeKind::Range(sub_type)
            }

            (TypType::Enum, _) => TypeKind::Enum(ty.enum_labels.iter().cloned().collect()),
            ...
            (TypType::Composite, _) => {
                ...
                TypeKind::Composite(attributes.into())
            }
        ...
        self.inner.cache_type_info.entry(ty.oid).or_insert_with(|| {
            TypeInfo(meta::Type::Custom(Arc::new(CustomType {
...
impl TypeResolver {
    ...
    async fn fill_cache(&mut self, conn: &mut Connection) -> Result<(), Error> {
...
impl<'r> FromRow<'r> for TypeResolverRow {
...
impl<'r> FromRow<'r> for ColumnResolverRow {
    fn from_row(row: &'r Row) -> Result<Self, Error> {
        Ok(Self {
...
impl<'r> Decode<'r> for TypCategory {
    fn decode(value: ValueRef<'r>) -> Result<Self, BoxDynError> {
        Ok(i8::decode(value)?.try_into()?)
    }
}

impl Type for TypCategory {
    fn type_info() -> TypeInfo {
        TypeInfo(meta::Type::Char)
    }
}
