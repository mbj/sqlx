...
impl Type for Polygon {
    fn type_info() -> TypeInfo {
        TypeInfo::with_name("polygon")
    }
}

impl HasArrayType for Polygon {
    fn array_type_info() -> TypeInfo {
        TypeInfo::with_name("_polygon")
    }
}

impl<'r> Decode<'r> for Polygon {
    fn decode(value: ValueRef<'r>) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        match value.format() {
            ValueFormat::Text => Ok(Polygon::from_str(value.as_str()?)?),
            ValueFormat::Binary => Ok(Polygon::from_bytes(value.as_bytes()?)?),
        }
    }
}

impl Encode<'_> for Polygon {
    fn produces(&self) -> Option<TypeInfo> {
        Some(TypeInfo::with_name("polygon"))
    }
...
mod polygon_tests {
    ...
    fn can_deserialise_polygon_type_bytes() {
        let polygon = Polygon::from_bytes(POLYGON_BYTES).unwrap();
        assert_eq!(
            polygon,
            Polygon {
                points: vec![
    ...
    fn can_deserialise_polygon_type_str_first_syntax() {
        let polygon = Polygon::from_str("[( 1, 2), (3, 4 )]").unwrap();
        assert_eq!(
            polygon,
            Polygon {
    ...
    fn cannot_deserialise_polygon_type_str_uneven_points_first_syntax() {
        let input_str = "[( 1, 2), (3)]";
        let polygon = Polygon::from_str(input_str);
    ...
    fn can_deserialise_polygon_type_str_float() {
        let polygon = Polygon::from_str("(1.1, 2.2), (3.3, 4.4)").unwrap();
        assert_eq!(
            polygon,
            Polygon {
