...
/// Wire-protocol primitive for `NotificationResponse` (`A`) frames.
///
/// Kept for downstream that revives LISTEN/NOTIFY handling. No in-tree consumer today —
/// `stream.rs` drops these frames without decoding.
#[allow(dead_code)]
#[derive(Debug, Clone)]
