...
impl Type for Box {
    fn type_info() -> TypeInfo {
        TypeInfo::with_name("box")
    }
}

impl HasArrayType for Box {
    fn array_type_info() -> TypeInfo {
        TypeInfo::with_name("_box")
    }
}

impl<'r> Decode<'r> for Box {
    fn decode(value: ValueRef<'r>) -> Result<Self, BoxDynError> {
        match value.format() {
            ValueFormat::Text => Ok(Box::from_str(value.as_str()?)?),
            ValueFormat::Binary => Ok(Box::from_bytes(value.as_bytes()?)?),
        }
    }
}

impl Encode<'_> for Box {
    fn produces(&self) -> Option<TypeInfo> {
        Some(TypeInfo::with_name("box"))
    }
...
impl Box {
    fn from_bytes(mut bytes: &[u8]) -> Result<Box, BoxDynError> {
...
mod box_tests {
    ...
    fn can_deserialise_box_type_bytes_in_order() {
        let pg_box = Box::from_bytes(BOX_BYTES).unwrap();
        assert_eq!(
            pg_box,
            Box {
                upper_right_x: 2.,
    ...
    fn can_deserialise_box_type_str_first_syntax() {
        let pg_box = Box::from_str("[( 1, 2), (3, 4 )]").unwrap();
        assert_eq!(
            pg_box,
            Box {
                upper_right_x: 1.,
    ...
    fn cannot_deserialise_too_many_numbers() {
        ...
        let pg_box = Box::from_str(input_str);
    ...
    fn can_deserialise_box_type_str_float() {
        let pg_box = Box::from_str("(1.1, 2.2), (3.3, 4.4)").unwrap();
        assert_eq!(
            pg_box,
            Box {
                upper_right_x: 1.1,
    ...
    fn can_order_box() {
        ...
        let pg_box = Box::from_bytes(&bytes).unwrap();
        assert_eq!(
            pg_box,
            Box {
                upper_right_x: 2.,
