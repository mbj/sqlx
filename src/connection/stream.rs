...
use crate::message::{
    BackendMessage, BackendMessageFormat, EncodeMessage, FrontendMessage, Notice,
    ParameterStatus, ReceivedMessage,
};
use crate::net::{self, BufferedSocket, Socket};
use crate::{ConnectOptions, DatabaseError, Severity};

// the stream is a separate type from the connection to uphold the invariant where an instantiated
// [Connection] is a **valid** connection to postgres

// when a new connection is asked for, we work directly on the [Stream] type until the
// connection is fully established

// in other words, `self` in any Connection method is a live connection to postgres that
...
impl Stream {
    pub(super) async fn connect(options: &ConnectOptions) -> Result<Self, Error> {
        ...
        self.inner
            .try_read(|buf| {
                ...
                let expected_len = message_len
                    ...
                    .ok_or_else(|| {
                        crate::err_protocol!("message_len + 1 overflows usize: {message_len}")
                    })?;
        ...
        loop {
            ...
            match message.format {
                BackendMessageFormat::ErrorResponse => {
                    ...
                    return Err(message.decode::<DatabaseError>()?.into());
                }

                BackendMessageFormat::NotificationResponse => {
                    // No LISTEN/NOTIFY consumer in this fork: ignore async NOTIFY frames.
                }
                ...
                BackendMessageFormat::NoticeResponse => {
                    ...
                    match notice.severity() {
                        Severity::Fatal | Severity::Panic | Severity::Error => {
                            tracing::error!(target: "sqlx::notice", message);
                        }
                        Severity::Warning => {
                            tracing::warn!(target: "sqlx::notice", message);
                        }
                        Severity::Notice => {
                            tracing::info!(target: "sqlx::notice", message);
                        }
                        Severity::Debug => {
                            tracing::debug!(target: "sqlx::notice", message);
                        }
                        Severity::Info | Severity::Log => {
                            tracing::trace!(target: "sqlx::notice", message);
                        }
                    }
                    continue;
                }
