...
impl Type for Path {
    fn type_info() -> TypeInfo {
        TypeInfo::with_name("path")
    }
}

impl HasArrayType for Path {
    fn array_type_info() -> TypeInfo {
        TypeInfo::with_name("_path")
    }
}

impl<'r> Decode<'r> for Path {
    fn decode(value: ValueRef<'r>) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        match value.format() {
            ValueFormat::Text => Ok(Path::from_str(value.as_str()?)?),
            ValueFormat::Binary => Ok(Path::from_bytes(value.as_bytes()?)?),
        }
    }
}

impl Encode<'_> for Path {
    fn produces(&self) -> Option<TypeInfo> {
        Some(TypeInfo::with_name("path"))
    }
...
mod path_tests {
    ...
    fn can_deserialise_path_type_bytes_closed() {
        let path = Path::from_bytes(PATH_CLOSED_BYTES).unwrap();
        assert_eq!(
            path,
            Path {
                closed: true,
    ...
    fn cannot_deserialise_path_type_uneven_point_bytes() {
        let path = Path::from_bytes(PATH_UNEVEN_POINTS);
        assert!(path.is_err());
    ...
    fn can_deserialise_path_type_bytes_open() {
        let path = Path::from_bytes(PATH_OPEN_BYTES).unwrap();
        assert_eq!(
            path,
            Path {
                closed: false,
    ...
    fn can_deserialise_path_type_str_first_syntax() {
        let path = Path::from_str("[( 1, 2), (3, 4 )]").unwrap();
        assert_eq!(
            path,
            Path {
                closed: false,
    ...
    fn cannot_deserialise_path_type_str_uneven_points_first_syntax() {
        let input_str = "[( 1, 2), (3)]";
        let path = Path::from_str(input_str);
    ...
    fn can_deserialise_path_type_str_float() {
        let path = Path::from_str("(1.1, 2.2), (3.3, 4.4)").unwrap();
        assert_eq!(
            path,
            Path {
                closed: true,
                points: vec![Point { x: 1.1, y: 2.2 }, Point { x: 3.3, y: 4.4 }]
            }
        );
    }
