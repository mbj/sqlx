...
use crate::codec::numeric::{Numeric, NumericSign};
...
impl HasArrayType for BigDecimal {
    fn array_type_info() -> TypeInfo {
        TypeInfo::NUMERIC_ARRAY
    }
}

impl TryFrom<Numeric> for BigDecimal {
    type Error = BoxDynError;

    fn try_from(numeric: Numeric) -> Result<Self, BoxDynError> {
        Self::try_from(&numeric)
    }
}

impl TryFrom<&'_ Numeric> for BigDecimal {
    type Error = BoxDynError;

    fn try_from(numeric: &'_ Numeric) -> Result<Self, Self::Error> {
        ...
        for (i, &digit) in digits.iter().enumerate() {
            if !Numeric::is_valid_digit(digit) {
                return Err(format!(
                    "Numeric to BigDecimal: {i}th digit is out of range {digit}"
                )
                .into());
            }
        ...
        let bigint = BigInt::from_radix_be(sign, &cents, 100)
            .ok_or("Numeric contained an out-of-range digit")?;

        Ok(BigDecimal::new(bigint, scale))
    }
}

impl TryFrom<&'_ BigDecimal> for Numeric {
    type Error = BoxDynError;
    ...
    fn try_from(decimal: &BigDecimal) -> Result<Self, BoxDynError> {
        ...
        let base_10_len = i64::try_from(base_10.len()).map_err(|_| {
            format!(
                "BigDecimal base-10 length out of range for Numeric: {}",
                base_10.len()
            )
        })?;
...
impl Encode<'_> for BigDecimal {
    fn encode_by_ref(&self, buf: &mut ArgumentBuffer) -> Result<IsNull, BoxDynError> {
        Numeric::try_from(self)?.encode(buf)?;

        Ok(IsNull::No)
    }

    fn size_hint(&self) -> usize {
        Numeric::size_hint(self.digits())
    }
}
...
impl Decode<'_> for BigDecimal {
    fn decode(value: ValueRef<'_>) -> Result<Self, BoxDynError> {
        match value.format() {
            ValueFormat::Binary => Numeric::decode(value.as_bytes()?)?.try_into(),
            ValueFormat::Text => Ok(value.as_str()?.parse::<BigDecimal>()?),
        }
    }
}

fn sign_to_pg(sign: Sign) -> NumericSign {
    match sign {
        Sign::Plus | Sign::NoSign => NumericSign::Positive,
        Sign::Minus => NumericSign::Negative,
    }
}
...
mod tests {
    use super::{BigDecimal, Numeric, NumericSign};
    ...
    fn one_hundred() {
        ...
        assert_eq!(
            Numeric::try_from(&one_hundred).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn ten_thousand() {
        ...
        assert_eq!(
            Numeric::try_from(&ten_thousand).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn two_digits() {
        ...
        assert_eq!(
            Numeric::try_from(&two_digits).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn one_tenth() {
        ...
        assert_eq!(
            Numeric::try_from(&one_tenth).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 1,
                weight: -1,
    ...
    fn one_hundredth() {
        ...
        assert_eq!(
            Numeric::try_from(&one_hundredth).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 2,
                weight: -1,
    ...
    fn twelve_thousandths() {
        ...
        assert_eq!(
            Numeric::try_from(&twelve_thousandths).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 3,
                weight: -1,
    ...
    fn decimal_1() {
        ...
        assert_eq!(
            Numeric::try_from(&decimal).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 4,
                weight: 0,
    ...
    fn one_digit_decimal() {
        ...
        assert_eq!(
            Numeric::try_from(&one_digit_decimal).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 8,
                weight: -2,
    ...
    fn issue_423_four_digit() {
        ...
        assert_eq!(
            Numeric::try_from(&four_digit).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn issue_423_negative_four_digit() {
        ...
        assert_eq!(
            Numeric::try_from(&negative_four_digit).unwrap(),
            Numeric::Number {
    ...
    fn issue_423_eight_digit() {
        ...
        assert_eq!(
            Numeric::try_from(&eight_digit).unwrap(),
            Numeric::Number {
                sign: NumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn issue_423_negative_eight_digit() {
        ...
        assert_eq!(
            Numeric::try_from(&negative_eight_digit).unwrap(),
            Numeric::Number {
                sign: NumericSign::Negative,
                scale: 0,
                weight: 1,
