...
impl Type for Cube {
    fn type_info() -> TypeInfo {
        TypeInfo::with_name("cube")
    }
}

impl HasArrayType for Cube {
    fn array_type_info() -> TypeInfo {
        TypeInfo::with_name("_cube")
    }
}

impl<'r> Decode<'r> for Cube {
    fn decode(value: ValueRef<'r>) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        match value.format() {
            ValueFormat::Text => Ok(Cube::from_str(value.as_str()?)?),
            ValueFormat::Binary => Ok(Cube::from_bytes(value.as_bytes()?)?),
        }
    }
}

impl Encode<'_> for Cube {
    fn produces(&self) -> Option<TypeInfo> {
        Some(TypeInfo::with_name("cube"))
    }
...
impl Cube {
    fn header(&self) -> Header {
        match self {
            ...
            Cube::ZeroVolume(values) => Header {
                is_point: true,
                dimensions: values.len(),
            },
            Cube::OneDimensionInterval(..) => Header {
                is_point: false,
                dimensions: 1,
            },
            Cube::MultiDimension(multi_values) => Header {
                is_point: false,
    ...
    fn from_bytes(mut bytes: &[u8]) -> Result<Self, BoxDynError> {
        ...
        match (header.is_point, header.dimensions) {
            (true, 1) => Ok(Cube::Point(bytes.get_f64())),
            (true, _) => Ok(Cube::ZeroVolume(
                read_vec(&mut bytes).map_err(|e| DecodeError::new(&header, e))?,
            )),
            (false, 1) => Ok(Cube::OneDimensionInterval(
                bytes.get_f64(),
                bytes.get_f64(),
            )),
            (false, _) => Ok(Cube::MultiDimension(read_cube(&header, bytes)?)),
        }
    }

    fn serialize(&self, buff: &mut ArgumentBuffer) -> Result<(), String> {
...
fn parse_point(str: &str) -> Result<Cube, Error> {
    Ok(Cube::Point(parse_float_from_str(
        str,
        "Failed to parse point",
    )?))
}

fn parse_zero_volume(content: &str) -> Result<Cube, Error> {
    content
        .split(',')
...
fn parse_one_dimensional_interval(point_vecs: Vec<&str>) -> Result<Cube, Error> {
...
fn parse_multidimensional_interval(point_vecs: Vec<&str>) -> Result<Cube, Error> {
    point_vecs
        .iter()
        .map(|&point_vec| {
            point_vec
...
impl Header {
    ...
    fn try_write(&self, buff: &mut ArgumentBuffer) -> Result<(), String> {
...
mod cube_tests {
    ...
    fn can_deserialise_point_type_byes() {
        let cube = Cube::from_bytes(POINT_BYTES).unwrap();
        assert_eq!(cube, Cube::Point(2.))
    }

    #[test]
    fn can_deserialise_point_type_str() {
        let cube_1 = Cube::from_str("(2)").unwrap();
        assert_eq!(cube_1, Cube::Point(2.));
        let cube_2 = Cube::from_str("2").unwrap();
        assert_eq!(cube_2, Cube::Point(2.));
    }

    #[test]
    fn can_serialise_point_type() {
        assert_eq!(Cube::Point(2.).serialize_to_vec(), POINT_BYTES,)
    }
    #[test]
    fn can_deserialise_zero_volume_bytes() {
        let cube = Cube::from_bytes(ZERO_VOLUME_BYTES).unwrap();
        assert_eq!(cube, Cube::ZeroVolume(vec![2., 3.]));
    }

    #[test]
    fn can_deserialise_zero_volume_string() {
        let cube_1 = Cube::from_str("(2,3,4)").unwrap();
        assert_eq!(cube_1, Cube::ZeroVolume(vec![2., 3., 4.]));
        let cube_2 = Cube::from_str("2,3,4").unwrap();
        assert_eq!(cube_2, Cube::ZeroVolume(vec![2., 3., 4.]));
    }

    #[test]
    fn can_serialise_zero_volume() {
        assert_eq!(
            Cube::ZeroVolume(vec![2., 3.]).serialize_to_vec(),
            ZERO_VOLUME_BYTES
        );
    }
    ...
    fn can_deserialise_one_dimension_interval_bytes() {
        let cube = Cube::from_bytes(ONE_DIMENSIONAL_INTERVAL_BYTES).unwrap();
        assert_eq!(cube, Cube::OneDimensionInterval(7., 8.))
    }

    #[test]
    fn can_deserialise_one_dimension_interval_string() {
        let cube_1 = Cube::from_str("((7),(8))").unwrap();
        assert_eq!(cube_1, Cube::OneDimensionInterval(7., 8.));
        let cube_2 = Cube::from_str("(7),(8)").unwrap();
        assert_eq!(cube_2, Cube::OneDimensionInterval(7., 8.));
    }

    #[test]
    fn can_serialise_one_dimension_interval() {
        assert_eq!(
            Cube::OneDimensionInterval(7., 8.).serialize_to_vec(),
    ...
    fn can_deserialise_multi_dimension_2_dimension_byte() {
        let cube = Cube::from_bytes(MULTI_DIMENSION_2_DIM_BYTES).unwrap();
        assert_eq!(
            cube,
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]])
        )
    }

    #[test]
    fn can_deserialise_multi_dimension_2_dimension_string() {
        let cube_1 = Cube::from_str("((1,2),(3,4))").unwrap();
        assert_eq!(
            cube_1,
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]])
        );
        let cube_2 = Cube::from_str("((1, 2), (3, 4))").unwrap();
        assert_eq!(
            cube_2,
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]])
        );
        let cube_3 = Cube::from_str("(1,2),(3,4)").unwrap();
        assert_eq!(
            cube_3,
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]])
        );
        let cube_4 = Cube::from_str("(1, 2), (3, 4)").unwrap();
        assert_eq!(
            cube_4,
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]])
        )
    ...
    fn can_serialise_multi_dimension_2_dimension() {
        assert_eq!(
            Cube::MultiDimension(vec![vec![1., 2.], vec![3., 4.]]).serialize_to_vec(),
    ...
    fn can_deserialise_multi_dimension_3_dimension_bytes() {
        let cube = Cube::from_bytes(MULTI_DIMENSION_3_DIM_BYTES).unwrap();
        assert_eq!(
            cube,
    ...
    fn can_deserialise_multi_dimension_3_dimension_string() {
        let cube = Cube::from_str("((2,3,4),(5,6,7))").unwrap();
        assert_eq!(
            cube,
            Cube::MultiDimension(vec![vec![2., 3., 4.], vec![5., 6., 7.]])
        );
        let cube_2 = Cube::from_str("(2,3,4),(5,6,7)").unwrap();
        assert_eq!(
            cube_2,
            Cube::MultiDimension(vec![vec![2., 3., 4.], vec![5., 6., 7.]])
        );
    ...
    fn can_serialise_multi_dimension_3_dimension() {
        assert_eq!(
            Cube::MultiDimension(vec![vec![2., 3., 4.], vec![5., 6., 7.]]).serialize_to_vec(),
