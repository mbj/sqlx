...
/// Possible sign values for [Numeric].
...
impl NumericSign {
    fn try_from_u16(val: u16) -> Result<Self, BoxDynError> {
        match val {
            SIGN_POS => Ok(NumericSign::Positive),
            SIGN_NEG => Ok(NumericSign::Negative),

            SIGN_NAN => unreachable!("sign value for NaN passed to NumericSign"),

            _ => Err(format!("invalid value for NumericSign: {val:#04X}").into()),
        }
    }
}
...
impl Numeric {
    ...
    pub const ZERO: Self = Numeric::Number {
    ...
    pub(crate) fn encode(&self, buf: &mut ArgumentBuffer) -> Result<(), String> {
        match *self {
            Numeric::Number {
                ref digits,
                sign,
                ...
                let digits_len = i16::try_from(digits.len()).map_err(|_| {
                    format!(
                        "Numeric digits.len() ({}) should not overflow i16",
                        digits.len()
                    )
                })?;
                ...
                for (i, &digit) in digits.iter().enumerate() {
                    if !Self::is_valid_digit(digit) {
                        return Err(format!("{i}th Numeric digit out of range: {digit}"));
                    }
