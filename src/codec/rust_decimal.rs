...
impl HasArrayType for Decimal {
    fn array_type_info() -> TypeInfo {
        TypeInfo::NUMERIC_ARRAY
    }
}

impl TryFrom<Numeric> for Decimal {
...
impl TryFrom<&'_ Numeric> for Decimal {
    type Error = BoxDynError;

    fn try_from(numeric: &'_ Numeric) -> Result<Self, BoxDynError> {
        ...
        match sign {
            NumericSign::Positive => value.set_sign_positive(true),
            NumericSign::Negative => value.set_sign_negative(true),
        }
...
impl From<&'_ Decimal> for Numeric {
...
mod tests {
    use super::{Decimal, Numeric, NumericSign};
    ...
    fn zero() {
        ...
        assert_eq!(Numeric::from(&zero), Numeric::ZERO,);

        assert_eq!(Decimal::try_from(&Numeric::ZERO).unwrap(), Decimal::ZERO);
    }
    ...
    fn decimal_4() {
        ...
        let expected_numeric = Numeric::Number {
            sign: NumericSign::Positive,
            scale: 5,
            weight: 1,
            digits: vec![1, 2345, 6789],
        };
        assert_eq!(Numeric::from(&decimal), expected_numeric);
    ...
    fn one_digit_decimal() {
        ...
        assert_eq!(Numeric::from(&one_digit_decimal), expected_numeric);
    ...
    fn max_value() {
        ...
        assert_eq!(Numeric::from(&Decimal::MAX), expected_numeric);
    ...
    fn mult_overflow() {
        ...
        let large_negative_number = Numeric::Number {
    ...
    fn max_value_max_scale() {
        ...
        assert_eq!(Numeric::from(&max_value_max_scale), expected_numeric);
    ...
    fn issue_423_negative_four_digit() {
        ...
        assert_eq!(
            Numeric::from(&negative_four_digit),
            Numeric::Number {
                sign: NumericSign::Negative,
                scale: 0,
                weight: 0,
    ...
    fn issue_423_negative_eight_digit() {
        ...
        assert_eq!(
            Numeric::from(&negative_eight_digit),
            Numeric::Number {
                sign: NumericSign::Negative,
                scale: 0,
                weight: 1,
    ...
    fn issue_2247_trailing_zeros() {
        ...
        assert_eq!(Numeric::from(&one_hundred), expected_numeric);
